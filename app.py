...
def encode_likert(df):
    ...
    # إجبار تحويل الأسئلة إلى أرقام (لكي لا تختفي الإحصائيات)
    for col in df_cleaned.columns:
        if col != 'Timestamp':
            try:
                df_cleaned[col] = pd.to_numeric(df_cleaned[col])
            except ValueError:
                pass # يترك البيانات الشخصية مثل (ذكر/أنثى) كنصوص
    return df_cleaned
...
if uploaded_file is not None:
    try:
        ...
        # --- إنشاء التبويبات (أصبحوا 6 تبويبات) ---
        ...
        with tab2:
            ...
            if numeric_cols:
                ...
                st.error("لم يتم التعرف على أسئلة مرقمة. تأكد من إجابات الاستبيان.")
        ...
        # ==========================================
        # التبويب الرابع: الفروق (T-test & ANOVA) مع حجم الأثر
        with tab4:
            st.subheader("⚖️ اختبار الفروق وحجم الأثر (Effect Size)")
            if categorical_cols and numeric_cols:
                group_col = st.selectbox("المتغير المستقل (فئات/ديموغرافي):", categorical_cols, key="dif_g")
                test_col = st.selectbox("المتغير التابع (السؤال المراد قياسه):", numeric_cols, key="dif_t")
                ...
                try:
                    if len(groups) == 2:
                        ...
                        st.markdown("### 📌 النتيجة وحجم الأثر:")
