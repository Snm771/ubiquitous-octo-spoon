...
"""
                        ...
                        # 👇 إضافة الشرح الأكاديمي تحت كل نتيجة (مطور وذكي) 👇
                        with st.expander(f"📝 عرض التفسير الأكاديمي للفرضية ({res_idx})"):
                            st.info(f"""
                            من خلال المعطيات الإحصائية أعلاه، يتضح أن مسار العلاقة بين المتغيرات قد **{proof}**، وهذا يعكس {'توافقاً' if is_accepted else 'اختلافاً'} مع الإطار النظري المفترض للدراسة. 
                            ويُمكن تفسير هذه النتيجة إحصائياً وعلمياً بأن {'تعزيز المتغير المستقل (أو أبعاده) ينعكس إيجابياً وبشكل ملحوظ على المتغير التابع، مما يستدعي استثمار هذه العلاقة' if is_accepted else 'المتغير المستقل بوضعه الحالي لا يشكل تأثيراً جوهرياً على المتغير التابع، مما يتطلب البحث عن عوامل ومؤثرات أخرى'}.
                            وتُعد هذه النتيجة ركيزة أساسية يُبنى عليها لاحقاً في توجيه التوصيات وصياغة القرارات المتعلقة بمشكلة الدراسة.
                            """)
                        res_idx += 1
                else:
