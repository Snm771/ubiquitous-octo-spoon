...
if uploaded_file is not None:
    try:
        ...
        if not analysis_cols:
            ...
            # ==========================================
            # 1. تبويب عينة الدراسة ✅ مع إضافة الرسوم المتعددة والمجموع
            # ==========================================
            with tab1:
                ...
                if categorical_cols:
                    for col in categorical_cols:
                        ...
                        with col1: 
                            ...
                            # ✅ خيارات الرسوم البيانية المتعددة
                            chart_options = ["أعمدة (Bar)", "دائري (Pie)", "دائري مجوف (Donut)", "خطي (Line)", "أعمدة أفقية (H-Bar)"]
                            chart_type = st.selectbox(f"اختر نوع الرسم لـ ({col}):", chart_options, key=f"chart_{col}")
                            
                        with col2: 
                            # ✅ عرض الرسم البياني حسب اختيار المستخدم (يتم تجاهل صف المجموع من الرسم)
                            if chart_type == "أعمدة (Bar)":
                                fig = px.bar(demo_df, x=demo_df.index, y='التكرار', text='التكرار', color=demo_df.index, height=350)
                            elif chart_type == "دائري (Pie)":
            ...
            with tab2:
                ...
                st.markdown("### 📝 التفسير الأكاديمي للتحليل الوصفي:")
            ...
            with tab3:
                ...
                if alpha_results:
                    ...
                    st.markdown("### 📝 التفسير الأكاديمي لمعامل الثبات:")
                        ...
                        try:
                            if len(grps) == 2:
                                ...
                                if len(valid_grps) < 2:
                                    ...
                                    if pval < 0.05: 
                                        st.success("✅ **توجد فروق ذات دلالة إحصائية.**")
                                        st.info(f"أظهرت المعطيات الإحصائية المستخلصة من تحليل التباين الأحادي (One-Way ANOVA) وجود فروق ذات دلالة إحصائية واضحة عند مستوى الدلالة ($\le 0.05$) بين استجابات أفراد العينة تُعزى للتصنيفات المختلفة في متغير ({g_col}) ضمن محور ({t_col}). يشير هذا المؤشر بقوة إلى أن تباين الاستجابات بين المجموعات المختلفة كان أكبر بكثير من التباين الداخلي لكل مجموعة على حدة. وهذا يؤكد أن المتغير الديموغرافي يلعب دوراً محورياً وجوهرياً في تشكيل آراء المبحوثين وتوجيه استجاباتهم.")
                                    else: 
                                        st.warning("⚠️ **لا توجد فروق ذات دلالة إحصائية.**")
                                        st.info(f"أوضحت المخرجات الإحصائية لتحليل التباين الأحادي (One-Way ANOVA) عدم وجود أي فروق ذات دلالة إحصائية عند مستوى الدلالة المرجعي ($\le 0.05$) بين فئات متغير ({g_col}) فيما يتعلق بتقييمهم لمحور ({t_col}). يعكس هذا الاستقرار الإحصائي إجماعاً عاماً وتوافقاً ملحوظاً من قبل أفراد العينة على فقرات هذا المحور، مما يقود إلى استنتاج علمي مفاده أن طبيعة الانتماء الفئوي لم تشكل عاملاً حاسماً في إحداث تباينات جوهرية في وجهات النظر.")
            ...
            with tab7:
                ...
                if not api_key:
                    ...
                    st.markdown("بناءً على الفهم أعلاه، حدد المتغيرات بدقة لكي ينفذ النظام الاختبار ويكتب التفسير:")
            ...
            # ==========================================
            # 8. التبويب الثامن: النتائج والتوصيات الذكية ✅
            # ==========================================
            with tab8:
                st.header("🎯 النتائج والتوصيات الذكية")
                st.markdown("يقوم النظام آلياً بفحص متوسطات الإجابات لكل فقرة/سؤال داخل المحاور، ويستخرج **النتائج** و**التوصيات** العلمية إما للتحسين (للفقرات الضعيفة) أو للمحافظة (إذا كانت جميعها مرتفعة).")
                ...
                for dim_name, cols in dimensions_dict.items():
                    if cols:
                        ...
                        # استخراج البيانات للنتيجة
                        dim_mean = round(item_means.mean(), 2)
                        highest_item_text = item_means.idxmax()
                        highest_mean_val = round(item_means.max(), 2)
                        lowest_item_text = item_means.idxmin()
                        lowest_mean_val = item_means.min()
                        
                        # صياغة النص للنتيجة
                        result_text = f"بلغ المتوسط العام لمحور ({dim_name}) درجة ({dim_mean} من 5). وقد حصلت فقرة '{highest_item_text}' على أعلى متوسط ({highest_mean_val})، بينما حصلت فقرة '{lowest_item_text}' على أقل متوسط ({lowest_mean_val})."
                        
                        low_items = item_means[item_means <= 3.50]
                        
                        if not low_items.empty:
                            # 🟢 الحالة 1: يوجد فقرات ضعيفة
                            ...
                            # 🔵 الحالة 2: لا يوجد فقرات ضعيفة
                            recommendations.append({
                                "المحور": dim_name,
                                "النتيجة": result_text,
                                "الفقرة": lowest_item_text,
                                "المتوسط": lowest_mean_val,
                ...
                if recommendations:
                    # عرض النتائج والتوصيات بشكل جميل
                    for idx, rec in enumerate(recommendations, 1):
                        with st.expander(f"📌 المحور: {rec['المحور']} (متوسط الفقرة المستهدفة: {rec['المتوسط']})", expanded=True):
                            st.markdown(f"**📊 النتيجة:** {rec['النتيجة']}")
                            st.markdown(f"**💡 التوصية:** {rec['التوصية']}")
                else:
