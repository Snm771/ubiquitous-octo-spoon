...
if uploaded_file is not None:
    try:
        ...
        if not analysis_cols:
            ...
            # ✅ التبويبات الثمانية (بما فيها التوصيات الذكية)
            ...
            # ==========================================
            # 1. تبويب عينة الدراسة ✅ مع إضافة الرسوم البيانية المتعددة والمجموع
            # ==========================================
            with tab1:
                ...
                if categorical_cols:
                    for col in categorical_cols:
                        ...
                        with col1: 
                            ...
                            chart_type = st.radio(f"اختر نوع الرسم لـ ({col}):", ["دائري (Pie)", "أعمدة (Bar)"], key=f"chart_{col}", horizontal=True)
                            
                        with col2: 
                            # ✅ عرض الرسم البياني حسب اختيار المستخدم (دائري أو أعمدة)
                            if chart_type == "دائري (Pie)":
                                ...
                                st.plotly_chart(px.bar(demo_df, x=demo_df.index, y='التكرار', text='التكرار', color=demo_df.index, height=350), use_container_width=True)
            ...
            with tab2:
                ...
                st.info("يهدف التحليل الوصفي المعروض في الجداول أعلاه إلى تشخيص مستوى الاستجابة لمتغيرات ومحاور الدراسة وفقاً لآراء أفراد العينة. بالاعتماد على مقياس النزعة المركزية المتمثل في **المتوسط الحسابي (Mean)**، يتم تحديد الاتجاه العام والميل الغالب لإجابات المبحوثين، حيث تشير القيم المرتفعة إلى تبلور رأي إيجابي، أو مستوى موافقة عالٍ، أو ممارسة قوية تجاه الفقرة أو المحور المدروس. وبالتوازي مع ذلك، يبرز دور مقياس التشتت المتمثل في **الانحراف المعياري (Standard Deviation)** كمؤشر إحصائي دقيق لقياس مدى تشتت أو تقارب تلك الآراء حول متوسطها؛ فالانحراف المعياري المنخفض (الذي يقترب من الصفر) يدل على تجانس فكري وتوافق ملحوظ بين المبحوثين، بينما الانحراف المرتفع يعكس تبايناً وتعدداً في وجهات النظر.")
            ...
            # ==========================================
            # 8. التبويب الثامن: التوصيات الذكية ✅ (ميزة جديدة)
            # ==========================================
            with tab8:
                st.header("💡 التوصيات الذكية (بناءً على المتوسطات الحسابية)")
                st.markdown("يقوم النظام آلياً بفحص متوسطات الإجابات لكل فقرة/سؤال داخل المحاور، ويستخرج التوصيات العلمية إما للتحسين (للفقرات الضعيفة) أو للمحافظة (إذا كانت جميعها مرتفعة).")
                
                recommendations = []
                
                # فحص كل محور تم اختياره
                for dim_name, cols in dimensions_dict.items():
                    if cols:
                        # حساب المتوسط لكل فقرة في هذا المحور
                        item_means = df_encoded[cols].mean()
                        
                        # تصفية الفقرات التي متوسطها أقل من أو يساوي 3.50
                        low_items = item_means[item_means <= 3.50]
                        
                        if not low_items.empty:
                            # 🟢 الحالة 1: يوجد فقرات ضعيفة (أقل من أو تساوي 3.5)
                            ...
                            # 🔵 الحالة 2: لا يوجد فقرات ضعيفة، نبحث عن الأقل قيمة
