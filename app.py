...
import numpy as np
import io  # 👈 إضافة مهمة جداً لحفظ الملف في الذاكرة

# استيراد مكتبة OpenAI
try:
    from openai import OpenAI
except ImportError:
    st.warning("جاري إعداد مكتبات الذكاء الاصطناعي...")
...
try:
    ...
    st.warning("⚠️ مكتبة الوورد غير مثبتة. يرجى إضافتها لملف requirements.txt")
...
if uploaded_file is not None:
    try:
        ...
        base_model = {
            ...
            "المتغير الوسيط (المعتمد)": ["البعد الأول", "البعد الثاني", "البعد الثالث", "البعد الرابع", "البعد الخامس", "البعد السادس"]
        }
        ...
        if not analysis_cols:
           ...
           # تعريف 6 تبويبات احترافية (بعد إزالة التبويبات اليدوية القديمة)
            ...
            with tab2:
                ...
                # حفظ جدول الوصفي في الذاكرة لملف الوورد (هذا هو الكود الصحيح هنا)
                st.session_state['desc_df_memory'] = desc_df[['العدد', 'المتوسط', 'الانحراف المعياري', 'الأدنى', 'الأقصى']]
...
""")
            ...
            with tab5:
                ...
                # تهيئة ذاكرة للتفسيرات المعمقة للفرضيات
                if 'deep_explanations' not in st.session_state:
                    st.session_state['deep_explanations'] = {}
                ...
                if st.session_state['hypothesis_history']:
                    ...
                    for h in st.session_state['hypothesis_history']:
                        ...
                        # صياغة الفرضية بناءً على قالب الأخت
                        hypo_text = f""" **الفرضية ({res_idx}):** ({h['text']})
...
"""
                        ...
                        # 👇 إضافة الشرح الأكاديمي وزر التعميق الذكي 👇
                        with st.expander(f"📝 عرض ومناقشة النتيجة للفرضية ({res_idx})"):
                            # التفسير المبدئي
                            st.info(f"""
                            من خلال المعطيات الإحصائية، يتضح أن مسار العلاقة بين المتغيرات قد **{proof}**، وهذا يعكس {'توافقاً' if is_accepted else 'اختلافاً'} مع الإطار النظري المفترض للدراسة. 
                            ويُمكن تفسير هذه النتيجة إحصائياً وعلمياً بأن {'تعزيز المتغير المستقل ينعكس إيجابياً على المتغير التابع' if is_accepted else 'المتغير المستقل بوضعه الحالي لا يشكل تأثيراً جوهرياً على المتغير التابع'}.
                            """)
                            
                            # زر التوليد المعمق
                            if st.button(f"✨ توليد مناقشة أكاديمية معمقة (AI) للفرضية ({res_idx})", key=f"btn_deep_{res_idx}"):
                                if not api_key:
                                    st.error("⚠️ يرجى إدخال مفتاح API في القائمة الجانبية.")
                                else:
                                    with st.spinner("جاري صياغة المناقشة والربط بالدراسات السابقة..."):
                                        prompt = f"""
                                        بصفتك باحثاً أكاديمياً خبيراً، قم بكتابة مناقشة وتفسير معمق لنتيجة الفرضية التالية:
                                        - نص الفرضية: {h['text']}
                                        - القرار الإحصائي: {decision} ({proof})
                                        
                                        المطلوب كتابة فقرتين رصينتين (جاهزة للنسخ في فصل مناقشة النتائج في رسالة ماجستير/دكتوراه):
                                        1. تفسير علمي ومنطقي لسبب ظهور هذه النتيجة في بيئة العمل أو عينة الدراسة.
                                        2. ربط افتراضي مع الدراسات السابقة (اذكر بشكل عام "وتتفق هذه النتيجة مع الدراسات التي أشارت إلى..." أو تختلف)، وماذا تعني هذه النتيجة تطبيقياً.
                                        """
                                        explanation = run_ai(prompt, api_key)
                                        st.session_state['deep_explanations'][f"hypo_{res_idx}"] = explanation
                            
                            # عرض التفسير المعمق إذا تم توليده
                            if f"hypo_{res_idx}" in st.session_state['deep_explanations']:
                                st.success(st.session_state['deep_explanations'][f"hypo_{res_idx}"])
            ...
            # ==========================================
            # 6. التبويب السادس: التوصيات (مستقل تماماً) ✅
            # ==========================================
            with tab6:
                st.header("💡 التوصيات الذكية والإجرائية")
                st.markdown("يقدم لك النظام توصيات مبدئية بناءً على القصور في المتوسطات الحسابية، مع إمكانية **توليد خطة إجرائية مفصلة** لكل توصية بضغطة زر.")
                
                # تهيئة ذاكرة للتوصيات المولدة الذكية لكي لا تختفي عند تحديث الصفحة
                if 'ai_recs_explanations' not in st.session_state:
                    st.session_state['ai_recs_explanations'] = {}

                if st.session_state['dim_recs']:
                    for idx, rec in enumerate(st.session_state['dim_recs'], 1):
                        # عرض التوصية الأساسية
                        st.success(f"**{idx}. المتغير/البعد:** {rec['dim']} | **المتوسط:** {rec['mean']}\n\n📌 {rec['rec']}")
                        
                        # زر التوليد الذكي بخطوة واحدة
                        col1, col2 = st.columns([1, 3])
                        with col1:
                            if st.button(f"✨ توليد خطة إجرائية للتوصية ({idx})", key=f"btn_rec_{idx}"):
                                if not api_key:
                                    st.error("⚠️ يرجى إدخال مفتاح API في القائمة الجانبية.")
                                else:
                                    with st.spinner("جاري صياغة الخطة التنفيذية والتبرير الأكاديمي..."):
                                        prompt = f"""
                                        بصفتك مستشاراً أكاديمياً وخبيراً استراتيجياً، لدينا نتيجة من تحليل إحصائي:
                                        - المحور/البعد: {rec['dim']}
                                        - المتوسط الحسابي: {rec['mean']} (من 5)
                                        - التوصية المبدئية: {rec['rec']}

                                        المطلوب كتابة تفصيل أكاديمي إجرائي (جاهز للنسخ في فصل التوصيات لرسالة ماجستير/دكتوراه) يحتوي على:
                                        1. فقرة توضح أهمية هذا المحور وسبب ضرورة تطبيق هذه التوصية بناءً على مستوى المتوسط الحسابي.
                                        2. إضافة عنوان "خطوات إجرائية مقترحة:" وتحته 3 خطوات عملية قابلة للتطبيق على أرض الواقع لتحقيق هذه التوصية.
                                        
                                        اجعل اللغة رصينة، علمية، ومقنعة.
                                        """
                                        explanation = run_ai(prompt, api_key)
                                        # حفظ النتيجة في الذاكرة لكي تبقى ظاهرة
                                        st.session_state['ai_recs_explanations'][f"rec_{idx}"] = explanation

                        # عرض التفسير إذا تم توليده مسبقاً
                        if f"rec_{idx}" in st.session_state['ai_recs_explanations']:
                            st.info(st.session_state['ai_recs_explanations'][f"rec_{idx}"])
                            
                        st.markdown("---")
                else:
                    st.warning("⚠️ يرجى زيارة تبويب (النتائج) أولاً لتوليد التوصيات المبدئية.")
