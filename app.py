...
# --- 1. دالة التشفير الذكي (القوة القصوى لعدم ضياع أي سؤال) ---
def encode_likert(df):
    ...
    # تنظيف شامل لكل الخلايا من المسافات المخفية
...
def smart_classify_columns(df):
    ...
    demo_keywords = ['عمر', 'سن', 'جنس', 'نوع', 'مؤهل', 'مرحلة', 'صف', 'خبرة', 'حالة', 'دخل', 'تخصص', 'عمل', 'البيانات الشخصية']
    
    for col in df.columns:
        if col.lower() == 'timestamp' or col == 'Unnamed: 0': continue
        
        # التأكد إذا كان العمود ديموغرافي
        is_demo = any(keyword in col for keyword in demo_keywords)
        
        if is_demo:
            categorical_cols.append(col)
        else:
            # إذا كان العمود رقمي أو يمكن تحويل أغلبه لأرقام، فهو سؤال
            converted = pd.to_numeric(df[col], errors='coerce')
            if converted.notnull().sum() > (len(df) * 0.3): # تم تخفيض النسبة لعدم تفويت أي سؤال
...
if uploaded_file is not None:
    try:
        ...
        st.sidebar.title("⚙️ إعدادات المحاور (Dimensions)")
        st.sidebar.info(f"تم اكتشاف {len(num_cols_auto)} سؤال بنجاح!")
        
        categorical_cols = st.sidebar.multiselect("👥 المتغيرات المستقلة (للمقارنة):", df_encoded.columns, default=cat_cols_auto)
        all_questions = [c for c in num_cols_auto if c not in categorical_cols]
        
        # التوزيع الذكي لملف "المشكلات السلوكية والمناخ الأسري" (20 سؤال للمحور الأول، والباقي للثاني)
        default_axis1 = all_questions[:20] if len(all_questions) >= 20 else all_questions
        default_axis2 = all_questions[20:] if len(all_questions) > 20 else []

        # بناء المحور الأول
        axis1_name = st.sidebar.text_input("اسم المحور الأول:", "المشكلات السلوكية")
        axis1_cols = st.sidebar.multiselect(f"أسئلة {axis1_name}:", all_questions, default=default_axis1)
        
        # بناء المحور الثاني
        axis2_name = st.sidebar.text_input("اسم المحور الثاني:", "المناخ الأسري")
        axis2_cols = st.sidebar.multiselect(f"أسئلة {axis2_name}:", all_questions, default=default_axis2)
        ...
        # المجموع الكلي للاستبيان (يشمل كل الأسئلة بلا استثناء)
        active_questions = axis1_cols + axis2_cols
        ...
        if not analysis_cols:
            ...
            st.success(f"✅ ممتاز! تم تجهيز الـ {len(active_questions)} سؤال بدقة.")
            ...
            with tab2:
                ...
                # 2. الإحصاء التفصيلي لكل فقرة (33 سؤال كاملة)
                st.markdown(f"### 2️⃣ الإحصاء التفصيلي لجميع فقرات الاستبيان (عددها: {len(active_questions)})")
                if active_questions:
                ...
                if categorical_cols:
                    ...
                    comp_axis = st.selectbox("اختر المحور أو الاستبيان ككل للمقارنة:", analysis_cols)
            ...
            with tab3:
                ...
                if len(active_questions) > 1:
                    ...
                    alpha_results.append({"المحور": "الاستبيان ككل", "عدد العبارات": len(active_questions), "معامل ألفا": round(a_total, 3)})
                
                if alpha_results:
                    st.dataframe(pd.DataFrame(alpha_results), use_container_width=True)
                    if 'a_total' in locals() and a_total >= 0.60: st.success("✅ أداة الدراسة تتمتع بثبات ممتاز ومقبول علمياً.")
            ...
            with tab4:
                ...
                if categorical_cols and analysis_cols:
                    g_col = st.selectbox("المتغير المستقل (مثال: النوع):", categorical_cols, key="g_f")
                    t_col = st.selectbox("المتغير التابع (المحور أو الكلي):", analysis_cols, index=len(analysis_cols)-1, key="t_f")
            ...
            with tab6:
                st.subheader("📈 تحليل الانحدار (Regression)")
                if len(analysis_cols) >= 2:
