...
# ✅ استيراد مكتبة Gemini بدلاً من OpenAI
try:
    import google.generativeai as genai
except ImportError:
    st.warning("⚠️ يرجى تثبيت مكتبة Gemini: pip install google-generativeai")

st.set_page_config(page_title="SmartStat Pro | الخبير الإحصائي", page_icon="📊", layout="wide")

# ==========================================
# --- دوال الذكاء الاصطناعي (AI Functions) - Gemini Version ---
# ==========================================

def analyze_hypothesis_text(text, api_key):
    """تحليل الفرضية الإحصائية باستخدام Gemini"""
    try:
        # ✅ تهيئة عميل Gemini
...
def generate_detailed_explanation(results, hypothesis, api_key):
    """توليد تفسير أكاديمي مفصل باستخدام Gemini"""
    try:
        genai.configure(api_key=api_key)
...
def get_table_explanation(table_string, context, api_key):
    """شرح الجداول الإحصائية باستخدام Gemini"""
    try:
        genai.configure(api_key=api_key)
        model = genai.GenerativeModel('gemini-1.5-flash')
        ...
        return f"⚠️ تعذر الاتصال بـ Gemini. تأكد من رصيد الـ API Key. تفاصيل الخطأ: {str(e)}"
...
# ==========================================
# ✅ سحب مفتاح Gemini API تلقائياً
# ==========================================
if "GEMINI_API_KEY" in st.secrets:
    api_key = st.secrets["GEMINI_API_KEY"]
else:
    st.sidebar.title("🤖 إعدادات الذكاء الاصطناعي (Gemini)")
    api_key = st.sidebar.text_input("🔑 مفتاح Gemini API:", type="password", 
                                    help="احصل على المفتاح من: https://aistudio.google.com/app/apikey",
...
if uploaded_file is not None:
    try:
        ...
        if not analysis_cols:
            ...
            # ==========================================
            # 7. التبويب السابع: محلل الفرضيات الذكي (Gemini Engine) ✅
            with tab7:
                st.header("🧠 المحلل الذكي للفرضيات (Gemini AI Engine)")
                st.markdown("ضع فرضية بحثك هنا، وسيقوم **جيميناي** بفهمها، واختيار الاختبار المناسب، وتنفيذه، وكتابة تقرير أكاديمي كامل لها!")
                
                if not api_key:
                    st.error("⚠️ يرجى إدخال مفتاح Gemini API في القائمة الجانبية أو إضافته في إعدادات التطبيق لتفعيل هذه الميزة الحصرية.")
                    st.info("💡 للحصول على المفتاح: انتقل إلى [Google AI Studio](https://aistudio.google.com/app/apikey) وأنشئ مفتاحاً مجانياً.")
                else:
                    user_hypothesis = st.text_area("✍️ أدخل نص الفرضية هنا:", "مثال: توجد علاقة ذات دلالة إحصائية بين جودة المعلومات والترويج للحدث.")
                    
                    if st.button("🔍 تحليل الفرضية آلياً عبر Gemini"):
                        with st.spinner("جاري فهم الفرضية عبر جوجل جيميناي..."):
                            try:
                    ...
                    if 'ai_analysis' in st.session_state:
                        st.success("✅ تم تحليل الفرضية بنجاح عبر جيميناي:")
                    ...
                    if st.button("🚀 تنفيذ وكتابة مناقشة النتائج (الفصل الرابع)"):
                        with st.spinner("جاري إجراء الاختبار وتأليف التفسير الأكاديمي المسهب عبر جيميناي..."):
    ...
    st.info("👆 ارفع ملف الإكسل أو CSV للبدء في التحليل الإحصائي الذكي")

# ==========================================
# تذييل الصفحة
# ==========================================
st.markdown("---")
st.caption("📊 SmartStat Pro © 2025 | مدعوم بـ **Google Gemini AI** للتفسير الأكاديمي الذكي")
