...
# محاولة استيراد مكتبة جوجل جيميناي بأمان
try:
...
def run_gemini(prompt, api_key):
    try:
        ...
        # استخدام الموديل Flash مع معالجة الخطأ المحتمل في التسمية
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(prompt)
        return response.text
    except Exception as e:
        # إذا فشل Flash، نحاول استخدام الموديل المستقر Pro كخطة بديلة
        try:
...
# ==========================================
# سحب مفتاح الذكاء الاصطناعي تلقائياً (تعديل لـ Gemini)
# ==========================================
if "GEMINI_API_KEY" in st.secrets:
    # إذا كان المفتاح مخزناً في السيرفر، استخدمه بصمت
    api_key = st.secrets["GEMINI_API_KEY"]
else:
    # إذا لم يكن موجوداً، أظهر مربع الإدخال في القائمة الجانبية
            ...
            with tab7:
                ...
                if not api_key:
                    ...
                    if st.button("🔍 تحليل الفرضية آلياً"):
                        with st.spinner("جاري فهم الفرضية عبر Gemini..."):
                            try:
                    ...
                    if st.button("🚀 تنفيذ وكتابة مناقشة النتائج (الفصل الرابع)"):
                        with st.spinner("جاري إجراء الاختبار وتأليف التفسير الأكاديمي المسهب عبر Gemini..."):
